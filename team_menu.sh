...
function download_files() {
	...
	# Add sliver-client_linux to PATH automatically
	if ! grep -q 'export PATH="$HOME/sliver:$PATH"' "$HOME/.bashrc"; then
		echo 'export PATH="$HOME/sliver:$PATH"' >> "$HOME/.bashrc"
		echo "[+] Added Sliver to PATH in .bashrc"
	fi

# Apply change in current shell session too
	export PATH="$HOME/sliver:$PATH"
