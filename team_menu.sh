...
function setup_vault() {
    local team="$1"
    local mount_dir="$HOME/share"
    local obsidian_appimage="$mount_dir/Obsidian-1.8.10.AppImage"

    if [[ "$team" == "Red" ]]; then
        if [[ -d "$mount_dir" ]]; then
            echo "[*] Launching Obsidian vault from mounted apt vault $mount_dir ..."
            if [[ -x "$obsidian_appimage" ]]; then
                "$obsidian_appimage" "$mount_dir" &
            else
                echo "[!] Obsidian AppImage not found or not executable at $obsidian_appimage"
            fi
        else
            echo "[!] Mounted apt vault directory $mount_dir not found."
        fi
    else
        # Purple team logic here, if any
    fi
}
