...
function download_and_move() {
    ...
    echo "[*] Downloading files from http://$SHARE_IP:8000/ ..."
    ...
    echo "[*] Moving downloaded folders and files to $HOME ..."
    cp -r "$TEMP_DIR/apt" "$HOME/"
    cp -r "$TEMP_DIR/tools" "$HOME/"
    cp -r "$TEMP_DIR/sliver" "$HOME/"
    cp "$TEMP_DIR/nc_host_oneliner.sh" "$HOME/"
